 ...
 *
 */
public class IO implements Serializable, PConstants {
	public static String currentDir;
	public static IO ioInstance;
  ...
  public static IO getInstance() {
  	if(ioInstance==null) ioInstance=new IO();
  	return ioInstance;
  }
...
//			Util.log("Save directory = " + filedialog.getDirectory()); 
			...
			if (returnVal==JFileChooser.APPROVE_OPTION) {
				...
				Util.log("getFilenameChooserDialog returned: "+s);
			} else {
				Util.logErr("Open command cancelled by user.");
			}
			
			
		} catch (Exception e) {
			Util.logErr("Exception: "+e.toString());
			e.printStackTrace();
		}
   ...
   */
	public static String getDiskVolumeName(String path) {
		...
		String res[]=Util.runtimeExec("cmd /c dir "+path, false);
		if(res!=null) {
...
//  	if(pos!=-1) name=name.substring(pos+1);
  ...
  public static String getExt(String name) {
  	...
  	if(pos!=-1) ext=name.substring(pos+1);
  	return ext;
  }
  ...
  public static String [] listFiles(String _path) {
  	...
  	try {
  		_path=IO.getAbsolutePath(_path);
...
//			if(list!=null) java.util.Arrays.sort(list);
	...
	public static void filterReset() {
		if(filter==null) filter=IO.getInstance().new MultiFilter();
		filter.reset();
	}
  ...
  public static String getFileSizeString(float len) {
    float kb=(float)len;
    if(kb<1000) {
      return Str.nf(kb,2,2)+" kb";
    }
    return Str.nf(kb/1024f,2,2)+" MB";
  }
...
//			filter.filterDirectoriesOnly();
			String [] list=listFiles(currentDir+DIRCHAR+"data");
...
//					println("getIncrementalFolder - "+e.toString());
			...
			last++;
			s=currentDir+DIRCHAR+"data"+DIRCHAR+prefix+Str.nf( last, padCnt);
  ...
  public void writeCSV(String filename,float val[][],char separator) {
      ...
      for (int i=0; i<rows; i++) {
        ...
        for (int j=0; j<cols; j++) {
          buf.append(Str.nf(val[i][j], 0, 5));
          if (j<511) buf.append(separator);
        }
        if (i%250==0||i==rows-1)
        	Util.log("Data frame "+Str.nf(i, cnt)+"/"+Str.nf(rows, cnt));
        str[i]=buf.toString();
      }
...
//        if (i%250==0||i==rows-1)
//        	println("Line "+Str.nf(i, 5)+"/"+Str.nf(rows, 5));
        rcnt++;
      } catch (Exception e) {
      	Util.log("Invalid row: "+e.toString());
      }
  ...
  public static InputStream getInputStream(String filename) {
  	...
  	try {
			...
			Util.logErr("getOutputStream - "+e.toString());
		}
		return in;
  }
  ...
  public static void getFilename(String filename) {
  	inFilename=IO.getAbsolutePath(filename);
  ...
  public static Object[] loadObjects(String filename) {
  	...
  	do {
  		tmp=loadObject();
  		if(tmp!=null) {
    		if(cnt==o.length) o=(Object [])Util.expandArray(o);
    		o[cnt++]=tmp;
  		}
  	} while(tmp!=null);
  	o=(Object [])Util.resizeArray(o, cnt);
  	return o;
  }
  
  public static boolean beginLoadObjects(String filename) {
  	getFilename(filename);
  	if(debugLevel>0) Util.log("Begin loading '"+inFilenameNoPath+"'");
  	filename=IO.getAbsolutePath(inFilename);
  	inCnt=0;
  ...
  public static Object loadObject() {
  	...
  	try {
			...
			Util.logErr("IO.loadObject(): "+e.toString());
		}
  ...
  public static void endLoadObjects() {
  	if(debugLevel>0) Util.log("Done loading '"+inFilenameNoPath+"' - "+inCnt+" objects read. "+
  			Str.timeStr(System.currentTimeMillis()-ioStartTime)+" elapsed.");
  	try {
			inObj.close();
			inObj=null;
		} catch (IOException e) {
  ...
  public static boolean beginSaveObjects(String filename) {
  	getFilename(filename);
		if(debugLevel>0) Util.log("Begin saving '"+inFilenameNoPath+"'");
  	...
  	try {
  		outObj=new ObjectOutputStream(getOutputStream(inFilename));
			...
			Util.logErr("IO.beginSaveObjects(): "+e.toString());
//			e.printStackTrace();			
			return false;
		}
  ...
  public static void saveObject(Serializable o) {
  	...
  	try {
			...
			inCnt++;
			if(debugLevel>0) if(inCnt%50==0) Util.log(inCnt+" objects saved.");
		} catch (Exception e) {
			Util.logErr("IO.saveObject(): "+e.toString());
		}
  }
  ...
  public static void saveObject(Serializable o[]) {
  	...
  	try {
  		for(int i=0; i<o.length; i++) {
  			...
  			inCnt++;
  			if(inCnt%50==0) Util.log(inCnt+" objects saved.");
  		}
		} catch (Exception e) {
			Util.logErr("IO.saveObject(): "+e.toString());
		}
  ...
  public static void endSaveObjects() {
  	try {    		
  		...
  		out.flush();
			...
			if(debugLevel>0) Util.log("Saved '"+inFilenameNoPath+"' - "+inCnt+" objects saved ("+fstr+"). "+
	  			Str.timeStr(System.currentTimeMillis()-ioStartTime)+" elapsed.");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
  ...
  public static void saveObject(Serializable o, String filename) throws IOException {
  	getFilename(filename);
    Util.log("Saving '"+inFilenameNoPath+"' "+o.getClass().getSimpleName());
  ...
  public static Object loadObject(String filename) throws IOException, ClassNotFoundException {
  	getFilename(filename);
    Util.log("Loading object stream '"+inFilenameNoPath+"'");
  ...
  public void endReadStrings() {
  	if(debugLevel>0) Util.log("Done loading '"+inFilenameNoPath+"' - "+inCnt+" lines read. "+
  			Str.timeStr(System.currentTimeMillis()-ioStartTime)+" elapsed.");
  	try {
			inReader.close();
			inReader=null;
...
//  	getFilename(filename);
  	...
  	if(!f.exists()) {
  		if(new File(filename+".gz").exists()) filename+=".gz";
  			...
  			Util.log("Could not find file '"+filename+"'");
  			return null;
  		}
  	}
  	...
  	do {
			ln=io.readString();
			if(ln!=null) {
				if (s.length==cnt) s=(String[])Util.expandArray(s);
				s[cnt++]=ln;
			}
		} while (ln!=null);
  	
  	io.endReadStrings();
		s=(String [])Util.resizeArray(s, cnt);
  ...
  public static void beginSaveStrings(String filename,boolean append) {
  	...
  	Util.log("beginSaveStrings '"+filename+"'");
  }

  public static void saveStrings(String str) {
  	if(outWriter==null) {
  		Util.logErr("saveStrings() - No output stream exists.");
  		return;
  	}
  	...
  	try {
			...
			outWriter.newLine();
			outCnt++;
			if(outCnt%50000==0) Util.log(outCnt+" lines - '"+str+"'");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
  ...
  public static void endSaveStrings() {
  	if(outWriter==null) {
  		Util.logErr("endSaveStrings() - No output stream exists.");
  		return;
  	}
  	try {
			outWriter.flush();

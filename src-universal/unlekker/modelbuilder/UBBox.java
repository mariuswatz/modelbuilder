...
public class UBBox {
	...
	private void addPoint(UVec3[] v, int n) {
		for(int i=0; i<n; i++) addPoint(v[i]);		
	}

	public UBBox add(UVec3 [] v, int n) {
		for(int i=0; i<n; i++) addPoint(v[i]);
		return this;
	}

	public UBBox add(UVec3 [] v) {
		return add(v,v.length);
	}
	
	public UBBox add(UVertexList vl) {

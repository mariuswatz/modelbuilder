...
//		UUtil.log(UUtil.toString(tok));
	...
	public static UVertexList getArc(float R,float a,float b,int res) {
		UVertexList vl=new UVertexList();
		float D=(b-a)/(float)(res-1);
		for(int i=0; i<res; i++) 
			vl.add(PApplet.cos(a+D*(float)i)*R,
					PApplet.sin(a+D*(float)i)*R,
					0);
	...
	static public void drawQuadstrip(PApplet p,UVertexList vl1,UVertexList vl2) {
		int id=0;
	...
	public void draw(PApplet p,int shapeType) {
...
//			for(int i=0; i<oldn; i++) v[i].set(vlOld.v[i]);
  ...
  public void add(PVector array[]) {
	...
	public void add(UVertexList vl, int start, int end) {
		if(end<start) 
			for(int i=start; i>=end; i--) add(vl.v[i]);
		else for(int i=start; i<=end; i++) add(vl.v[i]);

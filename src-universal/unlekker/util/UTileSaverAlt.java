...
public class UTileSaverAlt {
	...
	public UTGAProgressive tga;
	
	public UTileSaverAlt(PApplet _p,int num,String _filename) {
		p=_p;
		filename=p.savePath(_filename);
		UUtil.logDivider(this.getClass().getSimpleName());
	...
	public UTileSaverAlt(PApplet _p,int num,String _saveFormat,String _savePath) {
		p=_p;
	...
	public void tilingSetup(int num) {
		...
		tga=new UTGAProgressive(p, numTiles, UIO.noExt(savePath)+".tga");
...
//			buffer.set(x*p.width, y*p.height, p.g);
			...
			if(tile==numTilesTotal) {
				...
				UUtil.log("Saving to '"+UIO.noPath(tga.filename)+"'");

...
//	public static HashMap shapeTypes=null;
  ...
  static {
    ...
    rnd=new URnd(System.currentTimeMillis());
...
//  	log("mod "+mod+" dim "+tmp2.width+" "+tmp2.height);
  ...
  /**
	...
	public static float bezier1D(PApplet p,float bezpt[],float t) {
	  int bezn=((bezpt.length-1)/3);
//		UUtil.log(bezpt.length+" "+bezn);
	  t*=(float)(bezn);
	  int seg=(int)t;
	  if(seg==bezn) seg--;
	  t-=(float)seg;
...
//		log("bid "+bid+" t="+t+" "+tt+" "+bezpt.length);
		return p.bezierPoint(bezpt[bid++], bezpt[bid++], bezpt[bid++], bezpt[bid++], t);
//		log(tt+" "+t+" bid "+bid+" bezn "+bezn+" "+bezpt.length+" "+(bezn*t));
		
//    float t1 = 1.0f - t;
//    return bezpt[bid++]*t1*t1*t1 + 3*bezpt[bid++]*t*t1*t1 + 3*bezpt[bid++]*t*t*t1 + bezpt[bid++]*t*t*t;
	}
	/**
...
//		return s.substring(1,s.length()-1);		
	...
	public static float avg(float [] f) {
		double avg=0;
		for(float ff:f) avg+=ff;
		return (float)(avg/(double)f.length);
	}

	public static float min(float [] f) {
		float min=Float.MAX_VALUE;
		for(float ff:f) if(ff<min) min=ff;
		return min;
	}

	public static float max(float [] f) {
		float max=Float.MIN_VALUE;
		for(float ff:f) if(ff>max) max=ff;
		return max;
	}
	
	public static float sum(float [] f) {
		float val=0;
		for(float ff:f) val+=ff;
		return val;
	}

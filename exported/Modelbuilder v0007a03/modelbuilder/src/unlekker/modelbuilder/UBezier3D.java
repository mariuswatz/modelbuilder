...
public class UBezier3D { 
	...
	public UVec3 evalPoint(float _t) {
		float ct,D,ctsq,tsq,t,t0,t1,t2,t3;
		UVec3 v1,v2,v3,v4,vv=new UVec3();
	...
	public UBezier3D eval(int steps) {
		float ct,D,ctsq,tsq,t,t0,t1,t2,t3;
...
//		UUtil.log("bez "+numSeg+" "+steps);
		D=1f/(float)(steps);
		for(int j=0; j<numSeg; j++) {	
			if(j==numSeg-1) D=1f/(float)(steps-1);
...
//				a*t1*t1*t1 + 3*b*t*t1*t1 + 3*c*t*t*t1 + d*t*t*t;
//				lastEval=a*t0+3*b*t1+3*c*t2+d*t3;
